...
const CHECK_PORTS_IN_USE_REQUEST_URL = `${REQUEST_BASE_URL}/CheckPortsInUseRequest`
...
exports.sendStartPipelineRequest = (sip_rec_app_logger, payload) => {
    return axios.post(START_CALL_RECORDING_PIPELINE_REQUEST_URL, payload, axios_headers_config)
    .then((response) => {
...
exports.checkPortsInUse = (sip_rec_app_logger, ports) => {
    return axios.post(CHECK_PORTS_IN_USE_REQUEST_URL, { ports }, axios_headers_config)
    .then((response) => {
        const http_code = response.status;
        const { status, portsInUse } = response.data;
        if (http_code == 200 && status) {
            return portsInUse || [];
        }
        sip_rec_app_logger.error(`[CARBYNE][HTTP-API] Unexpected response checking ports in use with MFS: http_status_code=${http_code}, response=${JSON.stringify(response.data)}`);
        return [];
    })
    .catch((error) => {
        sip_rec_app_logger.error(`[CARBYNE][HTTP-API] Failed to check ports in use with MFS, proceeding without the check: ${error}`);
        return [];
    });
}
