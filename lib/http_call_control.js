...
exports.sendStartPipelineRequest = (sip_rec_app_logger, payload) => {
    const result = axios.post(START_CALL_RECORDING_PIPELINE_REQUEST_URL, payload, axios_headers_config)
    .then((response) => {
...
exports.sendStopPipelineRequest = (sip_rec_app_logger, payload) => {
    const result = axios.post(STOP_CALL_RECORDING_PIPELINE_REQUEST_URL, payload, axios_headers_config)
    .then((response) => {
        const http_code = response.status;
        const mfs_status = response.data.status;
        if (http_code == 200 && mfs_status) {
    ...
    .catch((error) => {
        if (error.response && 'data' in error.response) {
            ...
            sip_rec_app_logger.error(`[CARBYNE][HTTP-API] Failed to stop MFS pipeline: error=${error}`);
        }
        return false;
    });
    return result;
