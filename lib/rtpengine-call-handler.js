...
module.exports = (req, res) => {
  ...
  logger.info(`module.exports, opt=${opts}`); 
...
function allocateEndpoint(which, rtpEngine, opts) {
  logger.info(`allocateEndpoint, opt=${opts}`);  
  ...
  logger.info(`callDetails: ${opts.callDetails}`);
  logger.info(`rtpengine args for ${which}: ${JSON.stringify(args)}, sending to ${JSON.stringify(rtpEngine.remote)}`);
...
function respondToInvite(opts) {
    logger.info(`respondToInvite`);
    logger.info(`respondToInvite, opt=${opts}`);
...
function initiateMFSNewCall(opts) {
    logger.info(`initiateMFSNewCall=1, opts: ${opts}`);
    ...
    logger.info(`initiateMFSNewCall=2, xIngressLabel: ${xIngressLabel}`);

    getCallPortsPromise = sdpParser.getAllocatedPorts(rtpEngineCallerSDP, rtpEngineCalleeSDP);
    logger.info(`initiateMFSNewCall=3, rtpEngineCallerSDP: ${rtpEngineCallerSDP} ${rtpEngineCalleeSDP}`);
