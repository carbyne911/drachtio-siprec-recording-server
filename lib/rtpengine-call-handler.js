...
function stopRTPSession(rtpEngine, opts) {
    opts.logger.info(`[CARBYNE] Stopping RTPEngine for call-id=${req.get('Call-ID')}, Cisco-Guid=${req.get('Cisco-Guid')}`)
    rtpEngine.delete(rtpEngine.remote, opts.callDetails).then((response) => {
        opts.logger.info(`[CARBYNE] Closed RTPEngine response: call-id=${req.get('Call-ID')}, Cisco-Guid=${req.get('Cisco-Guid')}, response=${JSON.stringify(response)}`);
    });
} 
/******************  CARBYNE END SECTION  ********************/
...
function respondToInvite(opts) {
  ...
  .then(([callerPort, calleePort]) => {
      opts.logger.info(`[CARBYNE] Extracted ports successfully: ingressStreamPort=${callerPort}, egressStreamPort=${calleePort}, call-id=${req.get('Call-ID')}, Cisco-Guid=${req.get('Cisco-Guid')}`);
  ...
  opts.logger.info(`[CARBYNE][SIP-REC] Responding to INVITE with OK: call-id=${req.get('Call-ID')}, Cisco-Guid=${req.get('Cisco-Guid')}, \ncaller:\n${JSON.stringify(req.caller, null, 2)}, \ncallee:\n${JSON.stringify(req.callee, null, 2)}, \nrequestHeaders:\n${JSON.stringify(req.msg.headers, null, 2)}, \nrtpengineCallerSdp:\n${JSON.stringify(req.rtpengineCallerSdp, null, 2)}, \rtpengineCalleeSdp:\n${JSON.stringify(req.rtpengineCalleeSdp, null, 2)}`);
  /******************  CARBYNE END SECTION  ********************/
...
function onCallEnd(rtpEngine, opts) {
    /****************  CARBYNE START SECTION  ********************/
    opts.logger.info(`[CARBYNE][SIP-REC] SIP-REC Call dialog has ended: call-id=${req.get('Call-ID')}, Cisco-Guid=${req.get('Cisco-Guid')}`);
