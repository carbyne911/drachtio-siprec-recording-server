...
const MIN_CLOCK_RATE = 8000;
const MAX_CLOCK_RATE = 48000;
/******************  CARBYNE END SECTION  ********************/
...
function initiateMFSNewCall(opts) {
    ...
    Promise.all([getCallPortsPromise, getCodecsAndClockRatePromise]).then(([[callerPort, calleePort], [callerCodec, callerClockRate, calleeCodec, calleeClockRate]]) => {
        ...
        if (!validationError) {
            ...
            opts.logger.error(`[CARBYNE] Couldn't receive call properties properly - ${validationError} for: call-id=${opts.req.get('Call-ID')}, Cisco-Guid=${opts.req.get('Cisco-Guid')}`);
        }
    }).catch(error => {
        console.log(error);
    });
...
function isValidatedProperties(callerPort, calleePort, callerCodec, callerClockRate, calleeCodec, calleeClockRate) {
    ...
    } else if (callerClockRate < MIN_CLOCK_RATE || callerClockRate > MAX_CLOCK_RATE) {
        return `invalid callerClockRate value ${callerClockRate}`;
    } else if (calleeClockRate < MIN_CLOCK_RATE || calleeClockRate > MAX_CLOCK_RATE) {
